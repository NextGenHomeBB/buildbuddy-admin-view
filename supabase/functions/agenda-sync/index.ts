...
async function syncUserCalendar(supabase: any, settings: any) {
  ...
  // Sync Apple Calendar if enabled
  if (settings.apple_enabled) {
    await syncAppleCalendar(supabase, settings);
  }
}
...
async function syncAppleCalendar(supabase: any, settings: any) {
  console.log(`Syncing Apple Calendar for user: ${settings.user_id}`);
  ...
  try {
    ...
    // Push internal events to Apple Calendar if bidirectional sync
    if (settings.sync_direction === 'export_only' || settings.sync_direction === 'bidirectional') {
...
function parseCalDAVResponse(xmlText: string): any[] {
  const events: any[] = [];
...
async function processAppleEvent(supabase: any, userId: string, event: any) {
...
async function pushEventsToApple(supabase: any, settings: any, credentials: any) {
...
function createICalEvent(event: any): string {
