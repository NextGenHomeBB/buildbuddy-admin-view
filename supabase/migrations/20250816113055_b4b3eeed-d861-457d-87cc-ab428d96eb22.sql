-- Phase 3: RLS Policy Fixes & Advanced Access Controls
...
-- 3. Fix user_project_role RLS policies to prevent infinite recursion
...
-- Create new RLS policies for user_project_role without recursion
...
-- 4. Create audit trail for sensitive operations
...
-- 5. Create function for logging sensitive operations
...
-- 6. Create data integrity check function
...
CREATE OR REPLACE FUNCTION public.audit_sensitive_document_access()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Log access to documents with customer data
  IF TG_OP = 'SELECT' AND (
    NEW.client_email IS NOT NULL OR 
    NEW.client_phone IS NOT NULL OR 
    NEW.client_address IS NOT NULL
  ) THEN
    PERFORM log_sensitive_operation(
      ...
      jsonb_build_object(
        ...
        'has_client_email', NEW.client_email IS NOT NULL,
        'has_client_phone', NEW.client_phone IS NOT NULL,
        'has_client_address', NEW.client_address IS NOT NULL
      ),
      CASE 
        WHEN get_current_user_role() = 'admin' THEN 1
...
-- Note: We'll add the trigger activation in the next phase to avoid conflicts