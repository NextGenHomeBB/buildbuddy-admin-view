-- Drop and recreate the assign_workers_to_project function with proper column qualification
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (SELECT 1 FROM public.profiles pr WHERE pr.id = worker_id) THEN
      RAISE EXCEPTION 'Worker with ID % does not exist', worker_id;
    END IF;
  END LOOP;
  
  -- Process each worker assignment with explicit table aliases
  ...
  -- Return the assigned records with explicit table alias
  RETURN QUERY
  SELECT 
