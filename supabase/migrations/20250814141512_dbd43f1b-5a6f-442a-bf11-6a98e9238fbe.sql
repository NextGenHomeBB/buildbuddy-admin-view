...
SELECT 
  ...
  GREATEST(pp.updated_at, COALESCE(pmc.last_updated, pp.updated_at), COALESCE(plc.last_updated, pp.updated_at), COALESCE(pec.last_updated, pp.updated_at)) as last_updated
...
LEFT JOIN (
  SELECT phase_id, MAX(updated_at) as last_updated 
  FROM public.phase_material_costs 
  GROUP BY phase_id
) pmc_updated ON pp.id = pmc_updated.phase_id
LEFT JOIN (
  SELECT phase_id, MAX(updated_at) as last_updated 
  FROM public.phase_labor_costs 
  GROUP BY phase_id
) plc_updated ON pp.id = plc_updated.phase_id
LEFT JOIN (
  SELECT phase_id, MAX(updated_at) as last_updated 
  FROM public.phase_expenses 
  GROUP BY phase_id
) pec_updated ON pp.id = pec_updated.phase_id;
...
-- Add a notice function to warn about direct view access
...
AS $$
BEGIN
  -- Log unauthorized direct access attempt
  INSERT INTO public.security_audit_log (
    user_id, action, table_name,
    new_values, ip_address
  ) VALUES (
    auth.uid(), 
    'UNAUTHORIZED_DIRECT_VIEW_ACCESS_BLOCKED', 
    'phase_costs_vw',
    jsonb_build_object(
      'warning', 'Direct access to phase_costs_vw is not allowed',
      'required_function', 'get_phase_costs_secure',
      'security_level', 'critical'
    ),
    inet_client_addr()::text
  );
  
  RETURN 'SECURITY NOTICE: Direct access to phase_costs_vw is restricted. Use get_phase_costs_secure() function instead.';
END;
$$;