-- Phase 3: RLS Policy Fixes & Advanced Access Controls (Final Fix)
...
-- 2. Drop and recreate can_access_project function with proper parameters
...
-- 3. Recreate the project select policy
CREATE POLICY "project_select_policy"
ON projects FOR SELECT
USING (can_access_project(id, auth.uid()));

-- 4. Drop existing problematic RLS policies on user_project_role
...
-- 5. Create new RLS policies for user_project_role without recursion
...
-- 7. Create function for logging sensitive operations
...
-- 8. Create data integrity check function
