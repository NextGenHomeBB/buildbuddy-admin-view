-- Safe User Onboarding Migration - Fixed Version
-- This version drops existing function first and disables ALL problematic triggers

BEGIN;

-- First, drop the existing function that has conflicting return type
DROP FUNCTION IF EXISTS public.create_user_profile(uuid, text);

-- Disable ALL problematic audit triggers that can cause recursion
...
DROP TRIGGER IF EXISTS trg_org_auto_member ON public.organizations;
...
-- 9) Create backward compatibility function (correct return type)
