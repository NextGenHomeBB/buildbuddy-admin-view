...
-- Update functions that don't have proper search_path restrictions
...
-- 3. Add missing function for project access validation
...
USING (
  ...
  -- Log admin access to credentials
  (SELECT log_critical_security_event('ADMIN_CREDENTIAL_ACCESS', 'medium', 
    jsonb_build_object('target_user_id', user_id, 'credential_type', 'apple_calendar')) IS NULL)
);
...
CREATE TRIGGER monitor_apple_credentials_access
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.apple_calendar_credentials
...
CREATE TRIGGER monitor_oauth_tokens_access
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.calendar_oauth_tokens
  FOR EACH ROW EXECUTE FUNCTION monitor_sensitive_data_access();
...
INSERT INTO public.security_settings (setting_key, setting_value, description, created_by) VALUES
('credential_access_rate_limit', '{"max_attempts": 5, "window_minutes": 60}', 'Rate limit for credential access attempts', auth.uid()),
('session_timeout_minutes', '480', 'Session timeout in minutes (8 hours)', auth.uid()),
('failed_login_lockout', '{"max_attempts": 5, "lockout_minutes": 15}', 'Failed login attempt lockout settings', auth.uid())
