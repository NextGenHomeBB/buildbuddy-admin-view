-- Phase module polish migration
-- 1. Remove static progress column from project_phases (it will be calculated client-side)
ALTER TABLE project_phases DROP COLUMN IF EXISTS progress;

-- 2. Create phase_budget table if it doesn't exist
...
-- 3. Create phase_budget_v view for cost calculations
...
-- 4. Add RLS policy for phase_budget table
...
-- 5. Add RLS policy for phase_budget_v view
CREATE POLICY "Admins read phase budget view"
ON phase_budget_v FOR SELECT
USING (get_current_user_role() = 'admin');

-- 6. Add trigger to update updated_at on phase_budget
...
-- 7. Insert some sample budget data for existing phases
