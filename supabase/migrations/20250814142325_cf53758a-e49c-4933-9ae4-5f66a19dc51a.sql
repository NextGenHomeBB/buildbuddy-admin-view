...
-- 3. Enhanced credential security function
...
BEGIN
  ...
  IF NOT public.check_rate_limit_enhanced('apple_credential_delete', 2, 3600) THEN
    PERFORM public.log_critical_security_event(
  ...
  RETURN jsonb_build_object(
    ...
    'deleted_count', credential_count
  );
END;
$$;

-- 4. Fix database function security by adding proper search paths
CREATE OR REPLACE FUNCTION public.update_project_progress(project uuid)
RETURNS void
LANGUAGE sql
SECURITY DEFINER
SET search_path = 'public'
AS $$
  ...
  SET progress = (
    SELECT 100.0 * sum(progress * estimated_days) / GREATEST(sum(estimated_days), 1)
...
-- 6. Create enhanced security monitoring function
