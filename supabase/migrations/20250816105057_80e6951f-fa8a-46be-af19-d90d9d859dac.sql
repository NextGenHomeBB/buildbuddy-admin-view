-- Fix RLS infinite recursion and project assignment issues (part 3)

-- 1. Drop dependent policies first
DROP POLICY IF EXISTS "project_select_policy" ON projects;
DROP POLICY IF EXISTS "Project access via can_access_project function" ON user_project_role;
...
-- 4. Create security definer function to check project assignment permissions
...
-- 6. Create simplified RLS policies for user_project_role
...
-- 7. Recreate project select policy
CREATE POLICY "project_select_policy"
ON public.projects
FOR SELECT
USING (can_access_project(id, auth.uid()));

-- 8. Update existing projects to have proper org_id
