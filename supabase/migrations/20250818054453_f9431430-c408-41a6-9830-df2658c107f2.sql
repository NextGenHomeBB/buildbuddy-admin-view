-- Fix worker assignment infinite recursion issue - corrected version
-- First query existing policies to drop them all
DO $$
DECLARE
    policy_record RECORD;
BEGIN
    -- Drop all existing policies on user_project_role table
    FOR policy_record IN 
        SELECT schemaname, tablename, policyname 
        FROM pg_policies 
        WHERE schemaname = 'public' AND tablename = 'user_project_role'
    LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON public.user_project_role';
    END LOOP;
END $$;
